...
/*   Updated: 2024/04/29 23:44:42 by mboumlik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void map_chek_borders(t_long *so_long)
{
    ...
    while (i < so_long->map_y)
    {
        ...
        {
            if (so_long->map[0][j] != '1' || so_long->map[so_long->map_y - 1][j] != '1')
            {
                ft_printf("Error\nin map border in width");
                clean_mlx(so_long);
                exit(1);
            }
            if (so_long->map[i][0] != '1' || so_long->map[i][so_long->map_x - 1] != '1')
            {
...
void map_check_items(t_long *so_long) {
    ...
    while (i < so_long->map_y)
    {
        ...
        while (so_long->map[i][j] != '\n' && so_long->map[i][j] != '\0')
        {
            ...
            && so_long->map[i][j] != 'E' && so_long->map[i][j] != '1' && so_long->map[i][j] != '0')
            {
